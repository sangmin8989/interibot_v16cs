# 인테리봇 견적 에러 문제점 분석

**작성일**: 2025-12-17  
**프로젝트**: 인테리봇 견적 디버깅

---

## 🔴 현재 발생 중인 에러

### 에러 1: TypeError - grandTotal 접근 실패

**에러 메시지**:
```
TypeError: Cannot read properties of undefined (reading 'grandTotal')
```

**발생 위치**:
- `app/onboarding/estimate/page.tsx:660`
- `app/onboarding/estimate/page.tsx:1145` (UI 렌더링)
- `app/onboarding/estimate/page.tsx:1234` (UI 렌더링)

**에러 발생 코드**:
```typescript
// ❌ 문제 코드 (660번 줄)
console.log('✅ V3 견적 결과:', {
  평수: py,
  basic: formatWon(basicEstimate.summary.grandTotal),  // summary가 undefined
  standard: formatWon(standardEstimate.summary.grandTotal),
  argen: formatWon(argenEstimate.summary.grandTotal),
  premium: formatWon(premiumEstimate.summary.grandTotal),
})

// ❌ 문제 코드 (1145번 줄 - UI)
{formatPrice(estimate.summary.grandTotal)}

// ❌ 문제 코드 (1234번 줄 - UI)
{formatWon(currentEstimate.summary.grandTotal)}
```

---

## 🔍 근본 원인 분석

### 원인 1: 비동기 함수를 동기적으로 호출 ⚠️ **가장 심각**

**문제**:
- `calculateFullEstimateV3`는 `async` 함수인데 `await` 없이 호출됨
- Promise 객체가 반환되어 `summary` 속성이 없음

**코드 위치**: `app/onboarding/estimate/page.tsx:637-652`

```typescript
// ❌ 현재 코드 (에러 발생)
const basicEstimate = calculateFullEstimateV3({ ...baseInput, grade: 'BASIC' })
const standardEstimate = calculateFullEstimateV3({ ...baseInput, grade: 'STANDARD' })
const argenEstimate = calculateFullEstimateV3({ ...baseInput, grade: 'ARGEN' })
const premiumEstimate = calculateFullEstimateV3({ ...baseInput, grade: 'PREMIUM' })

// 결과: basicEstimate는 Promise 객체
// basicEstimate.summary → undefined (Promise에는 summary 속성이 없음)
```

**함수 정의 확인**:
```typescript
// lib/estimate/calculator-v3.ts:390
export async function calculateFullEstimateV3(input: EstimateInputV3): Promise<FullEstimateV3> {
  // ... 비동기 로직
  return {
    summary: {
      grandTotal: ...,
      // ...
    }
  }
}
```

### 원인 2: 안전 체크 부재

**문제**:
- `summary`가 `undefined`일 수 있는데 옵셔널 체이닝(`?.`) 미사용
- 기본값(`|| 0`) 제공 없음

**영향**:
- 계산 실패 시 UI 크래시
- 사용자 경험 저하

### 원인 3: 타입 정의와 실제 구현 불일치

**문제**:
- 타입 정의에는 `summary`가 필수로 정의되어 있음
- 실제로는 계산 실패 시 `summary`가 없을 수 있음

```typescript
// 타입 정의 (lib/estimate/calculator-v3.ts:244)
export interface FullEstimateV3 {
  summary: {  // 필수로 정의됨
    grandTotal: number;
    materialTotal: number;
    // ...
  };
}

// 실제 동작: 계산 실패 시 summary가 없을 수 있음
```

### 원인 4: 에러 처리 부재

**문제**:
- 계산 실패 시 `undefined` 반환
- UI에서 `undefined.summary.grandTotal` 접근 시 크래시
- 사용자에게 명확한 에러 메시지 제공 안 됨

---

## 📊 시스템 구조상 문제점

### 문제 1: 견적 엔진 이중화

**현재 상태**: 두 개의 견적 시스템이 혼재

1. **V3 계산기** (`lib/estimate/calculator-v3.ts`)
   - 4등급 시스템 (BASIC, STANDARD, ARGEN, PREMIUM)
   - 파일 기반 가격 데이터
   - `calculateFullEstimateV3` 함수 (async)
   - **현재 에러 발생 지점**

2. **헌법 v1 엔진** (`lib/estimate/constitution-v1-engine.ts`)
   - 단일 아르젠 기준
   - Supabase DB 기반
   - `generateEstimate` 함수

**영향**:
- 코드 중복 및 유지보수 어려움
- 일관성 없는 데이터 소스
- UI에서 두 시스템을 혼용하여 사용

### 문제 2: 데이터 흐름 불명확

```
사용자 입력 (Store)
  ↓
견적 계산 함수 (V3 또는 헌법 v1) ← 어느 것을 사용할지 불명확
  ↓
어댑터 레이어 (생성됐지만 미사용) ← 안전 변환 기회 놓침
  ↓
UI 렌더링 ← 에러 발생 지점
```

### 문제 3: 상태 관리 복잡성

**여러 Store가 복잡하게 상호작용**:
- `spaceInfoStore`: 평수, 방 개수 등
- `processStore`: 공정 선택
- `scopeStore`: 공간 선택

**문제**:
- Store 간 동기화 문제 가능
- `selectedSpaces: undefined` 같은 상태 발생 가능

---

## 🎯 해결 방안

### 즉시 수정 필요 (Critical)

#### 수정 1: await 추가

```typescript
// ✅ 수정 후
const basicEstimate = await calculateFullEstimateV3({ ...baseInput, grade: 'BASIC' })
const standardEstimate = await calculateFullEstimateV3({ ...baseInput, grade: 'STANDARD' })
const argenEstimate = await calculateFullEstimateV3({ ...baseInput, grade: 'ARGEN' })
const premiumEstimate = await calculateFullEstimateV3({ ...baseInput, grade: 'PREMIUM' })
```

#### 수정 2: 안전 체크 추가

```typescript
// ✅ 수정 후
console.log('✅ V3 견적 결과:', {
  평수: py,
  basic: formatWon(basicEstimate?.summary?.grandTotal || 0),
  standard: formatWon(standardEstimate?.summary?.grandTotal || 0),
  argen: formatWon(argenEstimate?.summary?.grandTotal || 0),
  premium: formatWon(premiumEstimate?.summary?.grandTotal || 0),
})

// ✅ UI 렌더링 수정
{formatPrice(estimate?.summary?.grandTotal || 0)}
{formatWon(currentEstimate?.summary?.grandTotal || 0)}
```

#### 수정 3: 에러 처리 추가

```typescript
try {
  const basicEstimate = await calculateFullEstimateV3({ ...baseInput, grade: 'BASIC' })
  // ... 나머지 계산
} catch (error) {
  console.error('❌ 견적 계산 에러:', error)
  setError('견적 계산 중 오류가 발생했습니다. 다시 시도해주세요.')
  // 안전한 기본값 설정
  setEstimates({
    basic: { summary: { grandTotal: 0, materialTotal: 0, laborTotal: 0 } },
    // ...
  })
}
```

### 중기 개선 (High Priority)

#### 개선 1: 어댑터 레이어 적용

```typescript
import { adaptEstimateResult } from '@/lib/adapters/estimate-result-adapter'

// 계산 후
const adaptedBasic = adaptEstimateResult(basicEstimate)
const adaptedStandard = adaptEstimateResult(standardEstimate)

// 사용 시 (안전함이 보장됨)
console.log('✅ V3 견적 결과:', {
  basic: formatWon(adaptedBasic.grandTotal),  // 항상 숫자
  // ...
})
```

#### 개선 2: 에러 바운더리 추가

```typescript
// React Error Boundary로 UI 크래시 방지
<ErrorBoundary fallback={<ErrorFallback />}>
  <EstimatePage />
</ErrorBoundary>
```

---

## 📝 재현 방법

1. 브라우저에서 `http://localhost:3001/onboarding/estimate` 접속
2. 공간 선택 (예: 주방)
3. 공정 선택 (예: 주방 코어, 조명/전기)
4. 견적 계산 버튼 클릭
5. **에러 발생**: 콘솔에 `TypeError: Cannot read properties of undefined (reading 'grandTotal')` 표시

---

## 🔗 관련 파일

- **에러 발생 파일**: `app/onboarding/estimate/page.tsx`
- **V3 계산기**: `lib/estimate/calculator-v3.ts`
- **헌법 v1 엔진**: `lib/estimate/constitution-v1-engine.ts`
- **어댑터**: `lib/adapters/estimate-result-adapter.ts` (생성됨, 미사용)
- **타입 정의**: `lib/types/헌법_견적_타입.ts`

---

## ✅ 수정 상태

- ✅ await 추가 완료
- ✅ 안전 체크 추가 완료
- ⚠️ 어댑터 적용 (선택사항)
- ⚠️ 에러 바운더리 추가 (미완료)

---

**마지막 업데이트**: 2025-12-17











