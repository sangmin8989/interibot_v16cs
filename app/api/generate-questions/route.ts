...
export async function POST(request: NextRequest) {
  try {
    ...
    // 2) 시스템 프롬프트 (V3.1 설계서 기반 질문 설계 규칙 + JSON 스키마)
    const systemPrompt = `
당신은 15년 경력의 프리미엄 인테리어 컨설턴트이자 심리 상담사입니다.
고객의 주거 정보와 라이프스타일을 바탕으로, **고객의 삶과 집의 상태를 분석해서 'Needs → Resolution → Action'으로 설계를 결정하는 인테리어 두뇌**가 되어야 합니다.
